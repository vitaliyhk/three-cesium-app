...
export class CesiumSceneExtension extends SceneExtension {
  ...
  cesiumTerrainProviderFactory: Promise<CesiumTerrainProvider> = new Promise(
  ...
  constructor(name: string, sceneModel: SceneModel) {
    ...
    const cesiumSceneNodes = getChildrenByType(sceneContent.props.children, [
      CesiumSceneContent,
    ]);

    // set the cesium scene or use the default
    const cesiumScene = cesiumSceneNodes?.[0];
  ...
  ): CesiumSceneViewBackgroundExtension {
    return new CesiumSceneViewBackgroundExtension(
      this.name,
      sceneViewModel,
      this
    );
  ...
  queryHeightAtLongitudeLatitude = async (
    longitude: number,
    latitude: number
  ): Promise<number> => {
  ...
  sampleTerrainHeightAtCartographicPositions = async (
    ...
    if (!this.cesiumTerrainProvider)
      throw new Error('Cesium Terrain Provider not initialized');
    return await sampleTerrainMostDetailed(
      this.cesiumTerrainProvider,
      cartographicPositionArray
    );
  };
}

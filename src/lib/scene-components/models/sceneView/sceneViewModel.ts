...
import {
  getSceneDirectionForSun,
  getScenePositionForSun,
} from '../../services';
...
export class SceneViewModel {
  ...
  sceneViewExtensions: Map<string, SceneViewExtension> = new Map<
    string,
    SceneViewExtension
  >();
  ...
  constructor(public name: string, public sceneModel: SceneModel) {
  ...
  moveCameraToCartesian(
    ...
    const scenePosition = new Vector3();
    this.sceneModel.getScenePositionForCartesian(
  ...
  setCameraTarget(
    ...
    return new Promise<void>((resolve, reject) => {
      const cameraPosition = this.cameraControls?.getPosition(
        new Vector3(),
        true
      );
      if (!cameraPosition) {
  ...
  passMouseEvent(e: MouseEvent) {
    if (!e) return;
    let handled = false;
    this.getSceneViewBackgroundExtensions().forEach(
      (sceneViewBackgroundExtension) => {
        if (handled) return;
